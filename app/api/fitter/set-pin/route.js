  ...
  try {
    ...
    if (!id) {
      const res = await fetch(
        `https://api.airtable.com/v0/${process.env.AIRTABLE_BASE_ID}/${encodeURIComponent(process.env.AIRTABLE_FITTERS_TABLE)}?filterByFormula=${encodeURIComponent(`{Email}="${email}"`)}`,
    ...
    const patchRes = await fetch(
      `https://api.airtable.com/v0/${process.env.AIRTABLE_BASE_ID}/${encodeURIComponent(process.env.AIRTABLE_FITTERS_TABLE)}/${id}`,
      {
        method: "PATCH",
    ...
    console.error("[fitter/set-pin]", err);
