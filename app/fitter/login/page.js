...
export default function FitterLogin() {
  ...
  const submitPin = async (digits) => {
    ...
    if (mode === "confirm") {
      ...
      setLoading(true);
      try {
        const res = await fetch("/api/fitter/set-pin", {
          method: "POST",
        ...
        if (data.success) {
          localStorage.setItem("strata_fitter_id", data.data.id);
          localStorage.setItem("strata_fitter_name", data.data.name);
          localStorage.setItem("strata_fitter_email", data.data.email);
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/fitter/verify-pin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const subs = {
    enter: "Enter your 6-digit PIN to continue.",
    set: "Choose a 6-digit PIN — you'll use this every time you log in.",
    ...
    <div style={{ minHeight: "100vh", background: BG, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui", padding: "20px" }}>
      <div style={{ width: "100%", maxWidth: 380, textAlign: "center" }}>
        ...
        {!emailSubmitted ? (
          ...
          <div>
            <h1 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 34, fontWeight: 400, color: TEXT, marginBottom: 10 }}>
              {headings[mode]}
            </h1>
              ...
              {activePin.map((d, i) => (
                <input
                  ...
                  style={{
                    ...
                    background: d ? "rgba(201,169,110,0.12)" : SURFACE,
                    border: `2px solid ${d ? GOLD : BORDER}`,
